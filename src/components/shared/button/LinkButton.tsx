...
export const ButtonLink = ({ onClick, classes, width="15rem", type="status", href, children, ...props}:ButtonProps) => {
    ...
    return (
        <div className='w-full mb-6 md:mb-0'>
            {href ? <Link className={`${linkClassNames}`} href={href || ""} onClick={onClick}>{children}</Link> : <button className={`${linkClassNames}`} onClick={onClick} {...props}>{children}</button>}
        </div>   
    );
};
